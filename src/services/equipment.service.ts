...
export const equipmentService = {
  ...
  async getEquipment(filters?: EquipmentFilters, page = 1, pageSize = 20): Promise<EquipmentListResponse> {
    try {
      ...
      if (error || !data) {
        return { data: [], count: 0, error: error || new Error('Ошибка загрузки оборудования') };
      }
  ...
  async getEquipmentById(id: string): Promise<{ data: Equipment | null; error: Error | null }> {
    try {
      const { data, error } = await get<{ data: Equipment }>(`/equipment/${id}`);

      if (error || !data) {
        return { data: null, error: error || new Error('Оборудование не найдено') };
      }
  ...
  async createEquipment(equipment: Omit<Equipment, 'id' | 'created_at' | 'updated_at' | 'qr_code'>): Promise<{ data: Equipment | null; error: Error | null }> {
    try {
      const { data, error } = await post<{ data: Equipment }>('/equipment', equipment);

      if (error || !data) {
        return { data: null, error: error || new Error('Ошибка создания оборудования') };
      }
  ...
  async updateEquipment(id: string, updates: Partial<Equipment>): Promise<{ data: Equipment | null; error: Error | null }> {
    try {
      const { data, error } = await put<{ data: Equipment }>(`/equipment/${id}`, updates);

      if (error || !data) {
        return { data: null, error: error || new Error('Ошибка обновления оборудования') };
      }
  ...
  async getEquipmentHistory(equipmentId: string): Promise<{ data: any[]; error: Error | null }> {
    try {
      const { data, error } = await get<{ data: any[] }>(`/equipment/${equipmentId}/history`);

      if (error || !data) {
        return { data: [], error: error || new Error('Ошибка загрузки истории') };
      }
  ...
  async changeOwner(
    ...
    try {
      ...
      if (error || !data) {
        return { data: null, error: error || new Error('Ошибка изменения владельца') };
      }
  ...
  async getEquipmentByLocation(department?: string, room?: string): Promise<{ data: Equipment[]; error: Error | null }> {
    try {
      const params = new URLSearchParams();
  ...
  async getEquipmentConsumables(equipmentId: string): Promise<{ data: any[]; error: Error | null }> {
    try {
      console.log('Запрос расходников для оборудования:', equipmentId);
      const { data, error } = await get<{ data: any[] }>(`/equipment/${equipmentId}/consumables`);
      ...
      console.log('Успешно загружено расходников:', data.data?.length || 0, data.data);
      return { data: data.data || [], error: null };
    } catch (error) {
