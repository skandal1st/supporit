...
import { handlePhoto, handleEquipmentCreateTicket, handleEquipmentHistory } from './handlers/photos.js';
import { handleTextMessage, handleCancelAction, setUserState } from './handlers/messages.js';
...
export async function initTelegramBot(): Promise<Telegraf<BotContext> | null> {
  ...
  if (!token) {
    console.log('[Telegram Bot] TELEGRAM_BOT_TOKEN не установлен, бот отключён');
    return null;
  }
  ...
  try {
    ...
    bot.action('tickets_list', requireLinkedAccount, (ctx) => handleTicketsList(ctx, 'all'));
    bot.action('my_tickets', requireLinkedAccount, handleMyTickets);
    bot.action('settings', requireLinkedAccount, showSettings);

    // Фильтры заявок
    bot.action('tickets_filter_all', requireLinkedAccount, (ctx) => handleTicketsList(ctx, 'all'));
    bot.action('tickets_filter_new', requireLinkedAccount, (ctx) => handleTicketsList(ctx, 'new'));
    ...
    bot.action(/^ticket_set_status_(.+)_(.+)$/, requireLinkedAccount, async (ctx) => {
      const ticketId = ctx.match[1];
    ...
    // Настройки
    bot.action('toggle_notifications', requireLinkedAccount, handleToggleNotifications);
    ...
    bot.action(/^equipment_create_ticket_(.+)$/, requireLinkedAccount, async (ctx) => {
    ...
    bot.action(/^equipment_history_(.+)$/, requireLinkedAccount, async (ctx) => {
      const equipmentId = ctx.match[1];
